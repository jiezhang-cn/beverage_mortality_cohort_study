water_turnover_calculate_data <- read.csv("C:/Users/zhangjie/Desktop/ckm_beverage_mortality/data/water_turnover_calculate_data_participant.csv")
...
# fat: 1g = 9 kcal, protein: 1g = 4 kcal, carbon: 1g = 4 kcal
...
# fat TEF: 2.5%, protein TEF: 25%, carbon TEF: 7.5%
...
for (i in 0:4) {
  ...
  temp_df$PA_met_hours <- rowSums(
    cbind(temp_df$vig_met_hours, temp_df$mod_met_hours, temp_df$lig_met_hours),
    na.rm = TRUE
  )
  
  all_na_activity_mets <- is.na(temp_df$vig_met_hours) & 
    is.na(temp_df$mod_met_hours) & 
    is.na(temp_df$lig_met_hours)
  temp_df$PA_met_hours[all_na_activity_mets] <- NA
  
  temp_df$PA_MET <- temp_df$PA_met_hours
  ...
  aaa[[paste0("PA_met_hours", i)]] <- temp_df$PA_met_hours
...
# PA_MET MET·hours
# equation：PA_en = PA_MET * weight
calc_PA_en <- function(PA_MET, weight) {
  ifelse(is.na(PA_MET) | is.na(weight), NA, PA_MET * weight) 
}
...
calc_TEE <- function(PA_en, TEF, BMR) {
  ifelse(is.na(PA_en) | is.na(TEF) | is.na(BMR), NA, BMR + TEF + PA_en)
}
...
## Reference: Circulation. 2013128:2259-2279 "Guide to the Assessment of Physical Activity: Clinical and Research Applications"
## 1 MET = 1 kcal·kg⁻¹·h⁻¹
##
## PA_MET = PA MET·hours 
## PA_en = PA_MET * weight (kcal/day)
## TEE = BMR + TEF + PA_en (kcal/day)
## PA_level = TEE / BMR

###########################################Water turnover calculation##########################################
assement_center <- read.csv("C:/Users/zhangjie/Desktop/Cohort Data/UKB/UKB_data/assement_center", sep="")
aaa <- merge(aaa,assement_center,all.x = T)
names(aaa)[153] <- "center"
...
ukb_GSODR_info <- read.csv("C:/Users/zhangjie/Desktop/ckm_beverage_mortality/data/ukb_GSODR_info.csv")
str(ukb_GSODR_info)
...
aaa %>%
  select(eid, weight, sex,
         starts_with("PA_level_M"),
         starts_with("RH"),
         starts_with("ELEVATION"),
         starts_with("age"),
         starts_with("TEMP"),
         starts_with("WTa_M"),
         starts_with("Drinking_watera")) %>%
  sample_n(20)
...
aaa %>%
  select(eid, weight, sex,
         ...
         starts_with("TEMP"),
         starts_with("WTb_M"),
         starts_with("Drinking_waterb")) %>%
  sample_n(20)



data.table::fwrite(aaa, "C:/Users/zhangjie/Desktop/ckm_beverage_mortality/data/ukb_water_turnover.csv")